...
async def list_products(
    ...
    query = select(Product).options(selectinload(Product.variants)).where(Product.is_active == True)
    ...
    if occasion and occasion.lower() != "all":
        query = query.where(func.lower(Product.occasion) == occasion.lower())

    if eggless is not None:
        query = query.where(Product.eggless_available == eggless)

    if featured is not None:
        query = query.where(Product.featured == featured)

    if bestseller is not None:
        query = query.where(Product.best_seller == bestseller)
