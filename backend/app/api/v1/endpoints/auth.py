...
from app.schemas.auth import TokenResponse, UserLogin, UserRegister, UserRegisterResponse, UserResponse, GoogleLogin
...
async def google_login(
    ...
    try:
        import os
        # Note: If no GOOGLE_CLIENT_ID is provided in the environment, we use a placeholder.
        # In a real production setup, you would strictly enforce audience verification.
        ...
        if not user:
            # Create a new user account automatically and mark as approved
            user = User(
                id=str(uuid.uuid4()),
                email=email,
                hashed_password=get_password_hash(str(uuid.uuid4())), # Random unguessable password
                full_name=full_name,
                role="CUSTOMER",
                is_active=True,
                is_verified=True,
                is_approved=True, # Auto approve Google Sign-In users
            )
            db.add(user)
            await db.commit()
            ...
            # If a user registered with email previously and was pending, auto-approve them now!
